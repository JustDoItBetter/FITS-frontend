...
fn run_db(ctx: SessionContext, df: DataFrame, commands: mpsc::Receiver<DbRequest>) {
    use DbCommand::*;
...
async fn create_backup(time: std::ops::Range<u64>, ret: mpsc::Sender<DbAnswer>, df: DataFrame) {
    if let Ok(res) = df.filter(col("timestamp").lt_eq(lit(time.end)).gt_eq(lit(time.start)))
        && let Ok(batches) = res.collect().await
    {
...
fn save(data: Vec<common::WeeklyReport>, ret: mpsc::Sender<DbAnswer>) {}
